...
class Request_model extends CI_Model {
	...
	// Add Signatory 4 here // Check
	function checkExist($strReqType = '', $strGenApplicant = '', $str1stOfficer='',$str2ndOfficer='',$str3rdOfficer='', $str4thOfficer='',$str5thOfficer='')
	{		
		...
		$this->db->where('Signatory4', $str4thOfficer);	
		$this->db->where('SignatoryFin', $str5thOfficer);			
	...
	// Add Signatory 4
	function get_signature($request_code,$secode='')
	{
		$this->load->model('Request_model');
		$signatories = $this->Request_model->request_signatories_bytype($request_code,$secode);
		...
		if(count($signatories) > 0):
			...
			if($signatories['Signatory4'] != ''):
				$signatory = explode(';',$signatories['Signatory4']);
				if(count($signatory) > 2):
					if($signatory[2] == $_SESSION['sessEmpNo']):
						$arremp_signature = array('Signatory4' => $_SESSION['sessEmpNo'], 'Sig4DateTime' =>date('Y-m-d'));
					endif;
				endif;
			endif;
	...
	/*
		Function updated by Loreto G. Gabawa Jr.
		10/26/2021
		loreto.gabawa.jr@gmail.com
	*/
	function get_next_signatory($ob,$type,$secode='')
	{		
		$this->load->helper('config_helper');

		$this->load->model('Request_model');		
		$signatories = $this->Request_model->request_signatories_bytype($type,$secode);
		//var_dump($signatories); exit;
		if(count($signatories) > 0):
			...
			$rflowsign_4 = $signatories['Signatory4'] != '' ? explode(';',$signatories['Signatory4']) : array('','','');
			$rflowsign_fin = $signatories['SignatoryFin'] != '' ? explode(';',$signatories['SignatoryFin']) : array('','','');
			//var_dump(strtolower($ob['requestStatus']));
			/*
				PROBLEM: 
				1. Request will change to CERTIFIED even the order of approval is not met, therefore once certified, it cannot proceed to the next signatory.
				2. This condition will proceed if 'file request' is the status
				3. The certify button is available for all admin
			*/
			if(strtolower($ob['requestStatus']) == 'filed request'):
				...
				if($ob['Signatory1'] == ''):
					...
					if($rflowsign_1[2] == ''):
						...
						if($rflowsign_2[2] == ''):
							...
							if($rflowsign_3[2] == ''):
								# check if flow_sign4 is null
								if($rflowsign_4[2] == ''):
									$display = $rflowsign_fin[2] == $_SESSION['sessEmpNo'] ? 1 : 0;
									return array('next_sign' => getDestination($signatories['SignatoryFin']), 'display' => $display, 'empNumber' => $rflowsign_fin[2], 'action' => $rflowsign_fin[0]);
								else:
									$display = $rflowsign_4[2] == $_SESSION['sessEmpNo'] ? 1 : 0;
									return array('next_sign' => getDestination($signatories['Signatory4']), 'display' => $display, 'empNumber' => $rflowsign_4[2], 'action' => $rflowsign_4[0]);
								endif;
							else:
								$display = $rflowsign_3[2] == $_SESSION['sessEmpNo'] ? 1 : 0;
								return array('next_sign' => getDestination($signatories['Signatory3']), 'display' => $display, 'empNumber' => $rflowsign_3[2], 'action' => $rflowsign_3[0]);
							endif;
						else:
							$display = $rflowsign_2[2] == $_SESSION['sessEmpNo'] ? 1 : 0;
							return array('next_sign' => getDestination($signatories['Signatory2']), 'display' => $display, 'empNumber' => $rflowsign_2[2], 'action' => $rflowsign_2[0]);
						endif;
					else:
						$display = $rflowsign_1[2] == $_SESSION['sessEmpNo'] ? 1 : 0;
						return array('next_sign' => getDestination($signatories['Signatory1']), 'display' => $display, 'empNumber' => $rflowsign_1[2], 'action' => $rflowsign_1[0]);
					endif;
				else:					
					# BEGIN SIGNATORY 2
					...
					if($ob['Signatory2'] == ''):
						...
						if($ob['Signatory3'] == ''):
							...
							# BEGIN SIGNATORY 4
							# check if ob_signatory4 is null	
							if($ob['Signatory4'] == ''):
								if($rflowsign_4[2] == ''):
									$display = $rflowsign_fin[2] == $_SESSION['sessEmpNo'] ? 1 : 0;
									return array('next_sign' => getDestination($signatories['SignatoryFin']), 'display' => $display, 'empNumber' => $rflowsign_fin[2], 'action' => $rflowsign_fin[0]);
				...
				$arr_signs = array($rflowsign_1[2],$rflowsign_2[2],$rflowsign_3[2],$rflowsign_4[2],$rflowsign_fin[2]);
				if(in_array($_SESSION['sessEmpNo'], $arr_signs)):
					return array('next_sign' => '', 'display' => 1);
				endif;
			endif;
		endif;
		return array('next_sign' => '', 'display' => 0); // HERE
	}
	...
	function request_signatories_bytype($request_type, $section_code = '')
	{
		if($request_type == 'TO'):
			$code = 'ALLEMP;'.$section_code.';';
			$res = $this->db->get_where('tblRequestFlow',array('Applicant' => $code))->result_array();		
			return count($res) > 0 ? $res[0] : array();
		else:
