...
<?php 
    ...
    $todate = date('Y-m-t', strtotime($year."-".$month."-1")); ; // Ending day for print preview (All)
                ...
                <div class="actions">
                    ...
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu pull-right">
                            ...
                            <!--<li>
                                <a href="<?=base_url('hr/attendance_summary/dtr/edit_mode').'/'.$arrData['empNumber'].'?datefrom='.$from.'&dateto='.$to.'&month-dtr='.$month.'&year-dtr='.$year?>">Edit Mode</a></li>-->
                            <li>
                    ...
                    <?php else: ?>
                        ...
                        <div class="btn-group">
                            <a class="btn green" href="javascript:;" data-toggle="dropdown" data-hover="dropdown" data-close-others="true"
                                style="font-size: 14px;padding: 5px 11px;"> Preview Actions
                                <i class="fa fa-angle-down"></i>
                            </a>
                            <ul class="dropdown-menu pull-right">
                                <li>
            ...
            <table class="table table-striped table-bordered order-column" id="tbldtr">
                <thead>
                    ...
                    <tr>
                        ...
                        <th rowspan="2" class="no-sort">Remarks</th>
                    </tr>
                    <tr>
                ...
                <tbody>
                    ...
                    <?php foreach($arremp_dtr as $index => $dtr ):?>
                        <tr class="odd <?=$dtr['day']?> <?=count($dtr['holiday_name']) > 0 ? 'holiday' : ''?>"
                            ...
                            <?php
                                foreach($arremp_eto as $etodata){
                                $requesttype = $etodata['requestCode'];
                                if ($requesttype == 'TO'):
                                                    $therequest = explode(';', $etodata['requestDetails']);
                                                    if(($dtr['dtrdate'] >= $therequest[1]) && ($dtr['dtrdate'] <= $therequest[2])){
                                                        echo "style='background: #61a7ff !important;'";
                                                    }
                                endif;
                            }
                            ?>
                            ...
                            <td><?=$dtr['day']?></td>
                            <td><?=($scanqr_dtr[$index]['inAM'] == null) ? "" : date("h:i A", strtotime($scanqr_dtr[$index]['inAM']))?></td>
                            <td><?=($scanqr_dtr[$index]['outBreak'] == null) ? "" : date("h:i A", strtotime($scanqr_dtr[$index]['outBreak']))?></td>
                            <td><?=($scanqr_dtr[$index]['inBreak'] == null) ? "" : date("h:i A", strtotime($scanqr_dtr[$index]['inBreak']))?></td>
                            <td><?=($scanqr_dtr[$index]['outPM'] == null) ? "" : date("h:i A", strtotime($scanqr_dtr[$index]['outPM']))?></td>
                            <td><?=($scanqr_dtr[$index]['inOvertime'] == null) ? "" : date("h:i A", strtotime($scanqr_dtr[$index]['inOvertime']))?></td>
                            <td><?=($scanqr_dtr[$index]['outOvertime'] == null) ? "" : date("h:i A", strtotime($scanqr_dtr[$index]['outOvertime']))?></td>

                            <?php
                                if($scanqr_dtr[$index]['underTime'] != null){
                                    $total_undertime += $scanqr_dtr[$index]['underTime'];
                                    $undertime = $scanqr_dtr[$index]['underTime'];
                                }else{
                                    $undertime = 0;
                                }

                                if($scanqr_dtr[$index]['late'] != null){
                                    $total_late += $scanqr_dtr[$index]['late'];
                                    $latetime = $scanqr_dtr[$index]['late'];
                                }else{
                                    $latetime = 0;
                                }
                            ?>
                            ...
                            <td><?=date('H:i', mktime(0,$latetime));?> </td>
                            <td><?/*=date('H:i', mktime(0,$undertime));*/ ?></td>
                            <td><strong>
                            <?php   
                            if(count($dtr['tos']) > 0):
                            ...
                            foreach($arremp_eto as $etodata){
                                ...
                                if ($requesttype == 'TO'):
                                    $therequest = explode(";", $etodata['requestDetails']);
                                         if(($dtr['dtrdate'] >= $therequest[1]) && ($dtr['dtrdate'] <= $therequest[2])):
                                             $requestdate = $etodata['SigFinDateTime'];
                                             $toyear = date('Y', strtotime($requestdate));
                                             echo "DENR6-".$toyear."-".$etodata['requestID'];                                       
                                         endif;
                                endif;
                            }
                        ?>
